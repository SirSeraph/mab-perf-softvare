let airemptmassOrg = document.getElementById("AirEmptMass_id");
let airemptmomentOrg = document.getElementById("AirEmptMoment_id");
let regNumber = document.getElementById("reg_choice_id");
...
//Nastavenie prídavných informácií onload
let namePICValue = "";
let dateValue = "";
let regNumberValue = regNumber.value;
regNumberValue = regNumberValue.substring(0, 2).toUpperCase() + '-' + regNumberValue.substring(2).toUpperCase();

//Seagle Air
regNumber.addEventListener('change', function() {
  const selectedValueOrg = this.value;
  if (selectedValueOrg === 'omhlk') {
      airemptmassOrg.value = 536;
      airemptmomentOrg.value = 167232;
  }
  //if (selectedValueOrg === 'omhlj') {
  //    airemptmassOrg.value = 524.5;
  //    airemptmomentOrg.value = 160590;
  //}
  if (selectedValueOrg === 'omkat') {
      airemptmassOrg.value = 506.1;
      airemptmomentOrg.value = 159927.6;
  }
  if (selectedValueOrg === 'omklh') {
      airemptmassOrg.value = 529;
      airemptmomentOrg.value = 159758;
  }
  if (selectedValueOrg === 'omklj') {
      airemptmassOrg.value = 524.5;
      airemptmomentOrg.value = 171511.5;
  }
  if (selectedValueOrg === 'omkll') {
      airemptmassOrg.value = 529;
      airemptmomentOrg.value = 159758;
  }
  if (selectedValueOrg === 'omkla') {
      airemptmassOrg.value = 511;
      airemptmomentOrg.value = 148190;
  }
  if (selectedValueOrg === 'omkls') {
      airemptmassOrg.value = 514.1;
      airemptmomentOrg.value = 152687.7;
  }
  if (selectedValueOrg === 'omklm') {
      airemptmassOrg.value = 522;
      airemptmomentOrg.value = 165996;
  }
})
...
regNumber.addEventListener('change', prepocet_imperialne_na_metricke);
...
regNumber.addEventListener('change', prepocet_metricke_na_imperialne);
...
regNumber.addEventListener('change', updateResults);
...
regNumber.addEventListener('change', limity);
...
regNumber.addEventListener('change', updateChart);
...
regNumber.addEventListener('change', updateInfo);

//Aktualizácia prídavných informácií po zmene
function updateInfo(){
  namePICValue = Name_PIC.value;
  dateValue = date.value;
  regNumberValue = regNumber.value;
  regNumberValue = (regNumberValue.substring(0, 2) + '-' + regNumberValue.substring(2)).toUpperCase();
}
...
function limity(){
  ...
  function exportDoPDF() {
    ...
    const chartDataUrl = chart.toBase64Image();
      ...
      doc.setFontSize(9);
      doc.setFont("Helvetica", "bold");    
      doc.text("mabap.netlify.app/seagleair", doc.internal.pageSize.width / 2, 35, { align: "center" }); // Posunieme os y do 35
      doc.setFontSize(6);
      doc.setFont("Helvetica");
      doc.setTextColor("gray");
      doc.text("Calculator made by Matúš Ištók", doc.internal.pageSize.width / 2, 280, { align: "center" }); 
      doc.text("Faculty of Aeronautics, Department of Flight Training, TUKE", doc.internal.pageSize.width / 2, 285, { align: "center" }); 
      doc.text("Košice 2024", doc.internal.pageSize.width / 2, 290, { align: "center" }); 
    //Uloženie PDF
...
regNumber.addEventListener('change', updateAccordingToTheFirst);
